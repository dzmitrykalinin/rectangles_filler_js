module.exports = class Drawer {
        ...
        if(dot1.isOnTheSameXLine(dot2)) {
            if (dot1.Y <= dot2.Y) {
                for(let i = dot1.Y; i <= dot2.Y; i++) {
                    image.Image[i][dot1.X].Color = color;
                }
            } else {
                for(let i = dot1.Y; i >= dot2.Y; i--) {
                    image.Image[i][dot1.X].Color = color;
        ...
        if(dot1.isOnTheSameYLine(dot2)) {
            if (dot1.X <= dot2.X) {
                for(let i = dot1.X; i <= dot2.X; i++) {
                    image.Image[dot1.Y][i].Color = color;
                }
            } else {
                for(let i = dot1.X; i >= dot2.X; i--) {
                    image.Image[dot1.Y][i].Color = color;
    ...
    static fillLine(image, dot, q, color) {
        ...
        while(w >= 0 && image.Image[dot.Y][w - 1] && image.Image[dot.Y][w - 1].isDefaultColor()) { w--; }
        // Go up to the east until w color is for replacing.
        while(e < image.Size && image.Image[dot.Y][e + 1] && image.Image[dot.Y][e + 1].isDefaultColor()) { e++; }
